...
function formatLiveEvent(event: RealtimeMessage): string {
  if (event.event === "analysis.intake_completed") return "Intake-аналіз завершено";
  if (event.event === "analysis.completed") return "Quick-аналіз завершено";
  if (event.event === "analysis.batch_completed") return "Batch-аналіз завершено";
...
export default function AnalyzePage() {
  ...
  const { status: realtimeStatus, events: realtimeEvents } = useRealtimeNotifications({ maxEvents: 8 });

  const intakeResult = intakeResults[activeResultIndex] ?? null;
  const liveTone = liveStatusTone(realtimeStatus);
  const liveAnalysisEvents = useMemo(
    () =>
      realtimeEvents
        .filter((item) => item.event.startsWith("analysis.") || item.event === "notification.test")
        .slice(0, 3),
    [realtimeEvents]
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div
        ...
        data-testid="analyze-live-panel"
        style={{ padding: "12px 16px", border: "1px solid rgba(255,255,255,0.06)" }}
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "10px", marginBottom: liveAnalysisEvents.length ? "10px" : "0" }}>
          <div style={{ fontSize: "13px", color: "var(--text-secondary)" }}>WS /api/notifications/ws</div>
          <span data-testid="analyze-live-status" style={{ fontSize: "12px", fontWeight: 700, color: liveTone.color }}>
            {liveTone.label}
          </span>
        </div>
        {liveAnalysisEvents.length > 0 && (
          <div data-testid="analyze-live-events" style={{ display: "grid", gap: "8px" }}>
            {liveAnalysisEvents.map((item) => (
              <div
                key={`${item.event}-${item.received_at}`}
                data-testid="analyze-live-event"
                style={{ display: "flex", justifyContent: "space-between", gap: "12px", fontSize: "12px", color: "var(--text-muted)" }}
              >
                <span style={{ color: "var(--text-secondary)" }}>{formatLiveEvent(item)}</span>
