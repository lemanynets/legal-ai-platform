...
import { type RealtimeMessage, type RealtimeStatus, useRealtimeNotifications } from "@/lib/realtime";
...
function liveStatusTone(status: RealtimeStatus): { color: string; label: string } {
  if (status === "connected") return { color: "var(--success)", label: "Live" };
  if (status === "connecting" || status === "reconnecting") return { color: "var(--warning)", label: "Connecting" };
  if (status === "error") return { color: "var(--danger)", label: "Error" };
  return { color: "var(--text-muted)", label: "Offline" };
}

function formatLiveGenerationEvent(event: RealtimeMessage): string {
  if (event.event === "generation.document_completed") return "Документ згенеровано";
  if (event.event === "generation.bundle_completed") return "Пакет документів згенеровано";
  if (event.event === "notification.test") return "Тестове сповіщення";
  return event.event;
}

function GeneratePageContent() {
  const searchParams = useSearchParams();
  const { status: realtimeStatus, events: realtimeEvents } = useRealtimeNotifications({ maxEvents: 10 });
  ...
  const liveTone = liveStatusTone(realtimeStatus);
  const liveGenerationEvents = useMemo(
    () =>
      realtimeEvents
        .filter((item) => item.event.startsWith("generation.") || item.event === "notification.test")
        .slice(0, 3),
    [realtimeEvents]
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div
        ...
        data-testid="generate-live-panel"
        style={{ padding: "12px 16px", border: "1px solid rgba(255,255,255,0.06)" }}
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "10px", marginBottom: liveGenerationEvents.length ? "10px" : "0" }}>
          <div style={{ fontSize: "13px", color: "var(--text-secondary)" }}>WS /api/notifications/ws</div>
          <span data-testid="generate-live-status" style={{ fontSize: "12px", fontWeight: 700, color: liveTone.color }}>
            {liveTone.label}
          </span>
        </div>
        {liveGenerationEvents.length > 0 && (
          <div data-testid="generate-live-events" style={{ display: "grid", gap: "8px" }}>
            {liveGenerationEvents.map((item) => (
              <div
                key={`${item.event}-${item.received_at}`}
                data-testid="generate-live-event"
                style={{ display: "flex", justifyContent: "space-between", gap: "12px", fontSize: "12px", color: "var(--text-muted)" }}
              >
                <span style={{ color: "var(--text-secondary)" }}>{formatLiveGenerationEvent(item)}</span>
                <span>{new Date(item.received_at).toLocaleTimeString("uk-UA", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</span>
              </div>
            ))}
          </div>
        )}
...
export default function GeneratePage() {
  return (
    <Suspense fallback={<div className="card-elevated" style={{ padding: 24 }}>Завантаження генератора...</div>}>
      <GeneratePageContent />
    </Suspense>
  );
}
